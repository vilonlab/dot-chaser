...
public class ExperimentEyeGaze : MonoBehaviour
{
    /// <summary>
    ...
    private void Update()
    {
        ...
        if (ApplyRotation)
        {
            var eyeRotation = CalculateEyeRotation(pose.orientation);
            transform.rotation = eyeRotation;
            Debug.Log($"Eye Rotation: {eyeRotation}");
            Debug.Log($"Eye Position: {pose.position}");

            // Trigger the event
            EyeRotationUpdated?.Invoke(eyeRotation, pose.position);
        }
